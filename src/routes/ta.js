...
router.post("/requests/:id/assignments", validate(assignSchema), asyncHandler(async (req, res) => {
  ...
  // ------------------------------------------------------------------------
  // RULE: A vehicle can be used for ONLY ONE route within the SAME request.
  // (Prevents assigning the same vehicle to multiple routes at the same time.)
  // ------------------------------------------------------------------------
  // 1) Prevent duplicates inside the payload itself
  const seen = new Set();
  for (const a of assignments) {
    const vid = Number(a.vehicle_id);
    if (seen.has(vid)) {
      throw httpError(400, "එකම වාහනය එකම රූට් එකට දෙවරක් assign කරන්න බැහැ");
    }
    seen.add(vid);
  }

  // 2) Prevent using a vehicle that is already assigned to ANOTHER route in this request
  const vehicleIdsToUse = Array.from(seen);
  if (vehicleIdsToUse.length > 0) {
    const conflict = await query(
      `SELECT ra.vehicle_id, v.vehicle_no, ra.route_id, r.route_no, r.route_name
       FROM request_assignments ra
       JOIN vehicles v ON v.id = ra.vehicle_id
       JOIN routes r ON r.id = ra.route_id
       WHERE ra.request_id = $1
         AND ra.vehicle_id = ANY($2::int[])
         AND ra.route_id <> $3
       LIMIT 1`,
      [requestId, vehicleIdsToUse, route_id]
    );
    if (conflict.rowCount > 0) {
      const c = conflict.rows[0];
      throw httpError(
        400,
        `මෙම වාහනය (${c.vehicle_no || ('ID ' + c.vehicle_id)}) දැනටමත් මෙම ඉල්ලීමේ වෙනත් රූට් එකකට assign කරලා තියෙනවා (Route ${c.route_no || c.route_id} - ${c.route_name || ''}). එකම request එකේ එක වාහනය route දෙකකට assign කළ නොහැක.`
      );
    }
  }
