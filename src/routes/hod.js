...
router.post("/employees", validate(employeeCreateSchema), asyncHandler(async (req, res) => {
  ...
  // Use a DB transaction so we don't create an employee without a matching user (when email+password is provided)
  const client = await pool.connect();
  try {
    await client.query("BEGIN");

    const empExists = await client.query("SELECT id FROM employees WHERE emp_no=$1", [emp_no]);
    ...
    let user = null;
    if (email && password) {
      const uExists = await client.query("SELECT id FROM users WHERE email=$1", [email]);
      ...
      // IMPORTANT: We are inside a transaction.
      // If an INSERT fails, Postgres marks the *whole transaction* as aborted until we ROLLBACK.
      // So we must use a SAVEPOINT to recover, then re-sync the users.id sequence and retry once.
      await client.query("SAVEPOINT sp_user_insert");
      try {
        const u = await client.query(insertUserSql, [email, hash, depId, emp.rows[0].id]);
        user = u.rows[0];
        await client.query("RELEASE SAVEPOINT sp_user_insert");
      } catch (e) {
        // roll back to savepoint to clear the aborted-state
        try { await client.query("ROLLBACK TO SAVEPOINT sp_user_insert"); } catch (_) {}

        // 23505 = unique_violation
        if (e && e.code === "23505" && (e.constraint === "users_email_key" || e.constraint === "users_email_idx")) {
          throw httpError(409, "Email already exists");
        }

        if (e && e.code === "23505" && e.constraint === "users_pkey") {
          // resync sequence safely (handles non-default sequence names)
          await client.query(
            "SELECT setval(pg_get_serial_sequence('users','id'), COALESCE((SELECT MAX(id) FROM users),0) + 1, false)"
          );
          // retry once (use a new savepoint)
          await client.query("SAVEPOINT sp_user_insert_retry");
          try {
            const u2 = await client.query(insertUserSql, [email, hash, depId, emp.rows[0].id]);
            user = u2.rows[0];
            await client.query("RELEASE SAVEPOINT sp_user_insert_retry");
          } catch (e2) {
            try { await client.query("ROLLBACK TO SAVEPOINT sp_user_insert_retry"); } catch (_) {}
            throw e2;
          }
        } else {
          throw e;
